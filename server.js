...
/*
  ...
  - Updated here to send address, postcode and city from the correct keys.
*/
async function syncToSheets(data) {
  try {
    ...
    const rows = data.map(item => [
      ...
      // Use the correct JSON keys for address, postcode and city:
      item.address || '',
      item.postcode || '',
      item.city || '',
...
async function fetchFromSheets() {
  try {
    ...
    return rows.slice(1).map(row => ({
      ...
      // Mapping back to our campaign properties using the JSON keys:
      address: row[7] || '',
      postcode: row[8] || '',
      city: row[9] || '',
...
app.post('/api/campaigns', async (req, res) => {
  ...
  try {
    ...
    const newCampaign = {
      ...
      // Updated to use JSON keys from the feed:
...
app.put('/api/campaigns/:campaignId', async (req, res) => {
  ...
  try {
    ...
    campaigns[index] = {
      ...
      // Updated to use the JSON keys for address, postcode and city:
      address: campaign.address || campaigns[index].address || '',
      postcode: campaign.postcode || campaigns[index].postcode || '',
      city: campaign.city || campaigns[index].city || '',
